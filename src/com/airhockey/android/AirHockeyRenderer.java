...
public class AirHockeyRenderer implements Renderer {
    ...
    private final float[] rotationAlpha = new float[36];
    ...
    public AirHockeyRenderer(Context context) {
        ...
        for(int step = 0; step <36; step++) {
        	rotationAlpha[step] = step * 10 - 180;
        }
    }
    ...
    public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
        ...
        sphere = new Sphere(new Point(0.5f, 1f, 0.5f), 0.2f);
	...
	public void handleTouchPress(float normalizedX, float normalizedY) {
		...
		//now test if this way intersects with the mallet by creating a 
	...
	public void handleTouchDrag(float normalizedX, float normalizedY) {
		...
		if(malletPressed) {
			Ray ray = convertNormalized2DPointToRay(normalizedX, normalizedY);
			//Define a plane representing out air hockey table.
			Plane plane = new Plane(new Point(0,0,0), new Vector(0, 1, 0));
			//Find out where the touched point intersects the plane
			//representing out table. We'll move the mallet along this plane.
